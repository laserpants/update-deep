...
resetQueue state = save { state | queue = [] }

incrementCounter : State -> Update State msg a
incrementCounter state = save { state | counter = 1 + state.counter }
...
dismissAfterSeconds n { id } =
  let secs = toFloat n
   in addCmd (Task.perform (always <| DismissOne id) (Process.sleep (secs * 1000)))

addNotification : String -> (Msg -> msg) -> State -> Update State msg a
addNotification text toMsg state =
  let notification = { id = Id state.counter, text = text }
   in state 
  ...
  let item { id, text } = 
   ...
   in
       div [ style "float" "right" ]
         ...
         , if List.isEmpty queue then text "" else div [ style "margin-top" ".5em" ] [ a [ href "#", onClick (toMsg DismissAll) ] [ text "Dismiss all notifications" ] ]
         ]
