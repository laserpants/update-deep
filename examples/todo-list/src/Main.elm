...
import Html.Attributes exposing (href, class, attribute, src, disabled, style, placeholder)
...
inTodos = inState { get = .todos, set = \state todos -> { state | todos = todos } }

inNotifications : In State Notifications.State msg a
inNotifications = inState { get = .notifications, set = \state notifs -> { state | notifications = notifs } }
