-- Functional programming, like mathematics, exposes complexity in a meticoluous and precise way.
...
postsCommentInit : Update PostsCommentModel b (PostsCommentMsg a)
postsCommentInit = save {}

postsCommentUpdate : PostsCommentMsg a -> PostsCommentModel -> Update PostsCommentModel a (PostsCommentMsg a)
postsCommentUpdate msg model =
  case msg of
    _ -> save model

postsCommentSubscriptions : PostsCommentModel -> Sub (PostsCommentMsg a)
...
insertPosts : HomePageModel -> ApiModel (List DataPost) -> HomePageModel 
insertPosts model posts = { model | posts = posts }

homePageInit : Maybe (List DataPost) -> Update HomePageModel b (HomePageMsg a)
homePageInit maybePosts =
   let resource = 
    ...
    in map HomePageModel
         (posts |> foldEvents |> mapCmd homePageApiMsg)
...
homePageUpdate msg model =
  case msg of
    ...
    Fetch ->
      model
        |> homePageUpdate (homePageApiMsg (Request "" Nothing))
...
homePageView model =
  ...
  let fetchButton = 
        div [] [ button [ onClick Fetch ] [ text "Fetch" ] ]
...
--pageInit = 
--  map HomePage (homePageInit Nothing)
--    |> mapCmd homePageMsg
--    |> foldEvents
...
postsCommentPageMsg : PostsCommentMsg a -> PageMsg a
postsCommentPageMsg = PostsCommentMsg << postsCommentUpdate
...
pageUpdate msg page =
  case ( msg, page ) of
    ...
    ( PostsCommentMsg update, PostsCommentPage postsCommentModel ) ->
      update postsCommentModel
        |> map PostsCommentPage
        |> mapCmd postsCommentPageMsg
    ( SetPage newPage, _ ) ->
      save newPage
    ( msg_, _ ) ->
      Debug.log ("Message not delivered: " ++ Debug.toString msg_) (save page)

pageSubscriptions : Page -> Sub (PageMsg a)
pageSubscriptions page = 
  case page of
    HomePage homePageModel ->
      Sub.map homePageMsg (homePageSubscriptions homePageModel)
    AboutPage ->
      Sub.none
    PostsCreatePage postsCreateModel ->
      Sub.map postsCreatePageMsg (postsCreateSubscriptions postsCreateModel)
    PostsShowPage postsShowModel ->
      Sub.map postsShowPageMsg (postsShowSubscriptions postsShowModel)
    PostsCommentPage postsCommentModel ->
      Sub.map postsCommentPageMsg (postsCommentSubscriptions postsCommentModel)
    LoginPage authLoginModel ->
      Sub.map loginPageMsg (authLoginSubscriptions authLoginModel)
    RegisterPage authRegisterModel ->
      Sub.map registerPageMsg (authRegisterSubscriptions authRegisterModel)

pageView : Page -> Html (PageMsg a)
pageView page = 
  case page of
    HomePage homePageModel ->
      Html.map homePageMsg (homePageView homePageModel)
    AboutPage ->
      text "about"
    PostsCreatePage postsCreateModel ->
      Html.map postsCreatePageMsg (postsCreateView postsCreateModel)
    PostsShowPage postsShowModel ->
      text "show post"
    PostsCommentPage postsCommentModel ->
      text "comment post"
    LoginPage authLoginModel ->
      Html.map loginPageMsg (authLoginView authLoginModel)
    RegisterPage authRegisterModel ->
      Html.map registerPageMsg (authRegisterView authRegisterModel)
...
handleRouteChange route model =
  case route of
    Just Home ->
      model
        |> appUpdate (pageMsg (HomePageMsg (always (homePageInit model.cache.posts))))
    Just About ->
      model
        |> appUpdate (pageMsg (SetPage AboutPage))
    Just PostsCreate ->
      model
        |> appUpdate (pageMsg (PostsCreateMsg (always postsCreateInit)))
    Just (Post id) ->
      save model
    ...
    Just Login ->
      model
        |> appUpdate (pageMsg (AuthLoginMsg (always authLoginInit)))
    Just Register ->
      model
        |> appUpdate (pageMsg (AuthRegisterMsg (always authRegisterInit)))
    Nothing ->
      save model
...
view model =
  ...
  , body  = [
      div []
        ...
        , hr [] []
        , Html.map pageMsg (pageView model.page)
        ]
    ]
  }
